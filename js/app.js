...
async function init() {
  try {
    ...
    // --- UPDATE NOTIFICATION ---
    // Show version modal automatically after an update
...
function setupEventListeners() {
      ...
      window.searchWorker.onmessage = function (e) {
        if (e.data.type === "results") {
          ...
          results.forEach((res) => {
            ...
            item.className =
              "p-3 hover:bg-slate-800 border-b border-slate-800 cursor-pointer transition-colors search-result-item";
  ...
  const handleNavInput = (value, type) => {
    ...
    if (type === "juz" && num >= 1 && num <= 30) {
      ...
      if (els.headerPageInput) els.headerPageInput.value = num;
    }
