/**
 * @file quranMeta.js
 * @description Quran metadata: Juz and Page boundaries (Medina Mushaf standard).
 * Source: Tanzil.net Quran Metadata v1.0 (CC BY 3.0)
 *
 * Each entry in JUZ_DATA and PAGE_DATA is [surahNumber, ayahNumber]
 * indicating the first ayah of that juz / page (1-indexed).
 */
 ...
 * @param {number} ayah  - Ayah number (1-based, absolute within surah)
 */
window.getJuzNumber = function (surah, ayah) {
  let juz = 1;
  for (let i = 1; i < JUZ_DATA.length; i++) {
    const [s, a] = JUZ_DATA[i];
...
/**
 * Returns the Page number (1–604) for a given surah and ayah (Medina Mushaf).
 * @param {number} surah - Surah number (1–114)
 * @param {number} ayah  - Ayah number (1-based, absolute within surah)
 */
window.getPageNumber = function (surah, ayah) {
  let page = 1;
  for (let i = 1; i < PAGE_DATA.length; i++) {
    const [s, a] = PAGE_DATA[i];
    if (s < surah || (s === surah && a <= ayah)) {
      page = i;
    } else {
      break;
...
// Build fast Set-based lookup maps: "surah:ayah" -> juz/page number
const _juzStartMap = new Map();
JUZ_DATA.forEach(([s, a], i) => { if (i > 0) _juzStartMap.set(`${s}:${a}`, i); });

const _pageStartMap = new Map();
PAGE_DATA.forEach(([s, a], i) => { if (i > 0) _pageStartMap.set(`${s}:${a}`, i); });

/**
 * Returns the juz number if this ayah starts a new juz, otherwise null.
 * @param {number} surah
 * @param {number} ayah
 */
window.getJuzStartAt = function (surah, ayah) {
  return _juzStartMap.get(`${surah}:${ayah}`) ?? null;
};

/**
 * Returns the page number if this ayah starts a new page, otherwise null.
 * @param {number} surah
 * @param {number} ayah
 */
window.getPageStartAt = function (surah, ayah) {
  return _pageStartMap.get(`${surah}:${ayah}`) ?? null;
};
